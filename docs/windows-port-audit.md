...
TV-facing status wording now reused by Android TV Settings, the Windows Cloud/RSS
panel, desktop scheduler display, and scheduler smokes. CloudDrive2 credential fields, sync-path fields, and RSS edit
...
builds and uploads a lightweight Windows desktop ZIP with
`:desktop-app:test :desktop-app:distZip -PbundleMpvRuntime=false`; nightly
main/master releases attach the same lightweight ZIP alongside the Android APK.
...
rejection, and selected subscription identity preservation. `:sync-engine-shared` also owns Bangumi metadata refresh/cache merge behavior through `BangumiMetadataRefreshCore`,
...
| Settings section contract is shared | `core:model/src/main/kotlin/com/miruplay/tv/model/SettingsSectionDisplayConventions.kt` owns the shared settings section enum plus Android TV and desktop orderings and section stepping helpers. Android TV `AddSourceScreen` and Windows `DesktopCloudRssPanel` now read the same section contract and shared titles/descriptions instead of carrying separate settings enums. `SettingsSectionDisplayConventionsTest` and `DesktopSettingsPanelTest` cover the shared contract and platform-specific navigation order. | Covered for settings menu copy/order |
