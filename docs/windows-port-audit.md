...
desktop-vs-TV UI parity beyond the Library and Details first screens remain
open.
...
| Make Windows UI visually match the TV UI | `ui-design`, `ui-tv/.../theme/Theme.kt`, `MiruPlayDesktopComposeApp.kt`, root `checkUiPaletteDrift`, `tools/capture-desktop-ui.ps1`, `build/desktop-ui-qa/*.png`, `build/android-tv-qa/library-baseline.png` | TV and desktop now derive the MiruPlay red/dark/blue/text/card palette from the same `MiruPlayPalette` constants. `checkUiPaletteDrift` fails if TV or desktop UI reintroduces raw shared palette literals outside `:ui-design`. Android TV Library baseline was captured from emulator `10.137.32.118:5555`. Windows Library now uses the TV-style full-width Explore header, right-side actions, TV empty state, and a poster-wall first screen after scanning local media. Poster selection now routes directly to Details, and Details starts with a TV-style hero using a large poster/backdrop surface, title context, plot, Play, and Back-to-poster-wall actions. Screenshot QA covers Library/Details/Player/Settings and asserts window size, dark palette, red accent, readable light text, visual diversity, and per-section distinctness. | Player/Settings and deeper navigation parity still need Android TV comparison; remote/source management controls remain desktop-specific. |
...
| Windows visual language matches TV | `ui-design` now owns the shared MiruPlay palette; Android TV `Theme.kt` and Compose Desktop both derive `AnimeRed`, `DarkBg`, `DarkSurface`, `AccentBlue`, `TextPrimary`, `TextSecondary`, and `CardBg` from `MiruPlayPalette`. The root `checkUiPaletteDrift` task guards against raw palette literal drift in `ui-tv/src` and `desktop-app/src`. Android TV Library was captured on emulator `10.137.32.118:5555`, showing the full-width Explore header, right-side Scan/Settings actions, and centered empty library state. Compose Desktop Library now mirrors that direction and shows poster-wall cards as the main scanned-library surface instead of a right-side indexed list. Compose Desktop Details now opens directly from a poster click and starts with a TV-style hero for poster/backdrop, title, context, plot, Play, and Back-to-poster-wall actions. Local screenshot QA covers Library, Details, Player, and Settings first screens; it asserts minimum TV-style window size, non-tiny PNG output, sampled visual diversity, dark-theme coverage, MiruPlay red accent pixels, readable light text pixels, and distinct images per section. Player/Settings still need more TV-parity work. | Partial |
...
adb exec-out screencap -p > build\android-tv-qa\library-baseline.png
...
1. Continue narrowing full Android TV screen parity gaps beyond the Library and
   Details first screens, especially Player, Settings, and navigation shape.
