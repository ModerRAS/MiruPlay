...
movement. Settings summary quick-action rows now also have TV-style horizontal
...
1. Continue narrowing TV parity gaps in less-traveled keyboard/DPAD focus paths outside the now-covered Android TV Settings category/page traversal, Settings summary quick-action rows, source-management local/remote fields plus actions, Cloud/RSS credential/sync/RSS edit fields plus action/toggle/subscription rows, desktop route rail, Library poster wall and highest-heat/recent shelf horizontal plus vertical movement, Details hero actions, Details hero-to-episodes/recents/Bangumi/media-details fallback plus episode-shelf season selector, remote browser list including parent navigation, Player transport controls plus top-return focus, Player playback-setting row movement, Player runtime form/check movement, saved-source card movement, and Bangumi metadata lists/action grid/apply-clear flow.
