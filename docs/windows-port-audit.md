...
desktop-vs-TV UI parity remain open.
...
| Windows visual language matches TV | `ui-design` now owns the shared MiruPlay palette; Android TV `Theme.kt` and Compose Desktop both derive `AnimeRed`, `DarkBg`, `DarkSurface`, `AccentBlue`, `TextPrimary`, `TextSecondary`, and `CardBg` from `MiruPlayPalette`. The root `checkUiPaletteDrift` task guards against raw palette literal drift in `ui-tv/src` and `desktop-app/src`. Android TV Library was captured on emulator `10.137.32.118:5555`, showing the full-width Explore header, right-side Scan/Settings actions, and centered empty library state. Compose Desktop Library now mirrors that direction and shows poster-wall cards as the main scanned-library surface instead of a right-side indexed list. Local screenshot QA covers Library, Details, Player, and Settings first screens; it asserts minimum TV-style window size, non-tiny PNG output, sampled visual diversity, dark-theme coverage, MiruPlay red accent pixels, readable light text pixels, and distinct images per section. Details/Player/Settings still need more TV-parity work. | Partial |
...
1. Continue narrowing full Android TV screen parity gaps beyond the Library
   first screen, especially Details, Player, and navigation shape.
