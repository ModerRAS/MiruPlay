...
Enter, Details hero Down to the episode shelf, Details hero Down to Continue
watching Enter, Details hero Down to Bangumi Use selected when no recents exist,
...
| Details hero keyboard/DPAD navigation | `DesktopDetailsPanels.kt`, `DesktopDetailHeroTest.kt`, `tools/smoke-desktop-local-source-ui.ps1`, `build/desktop-local-source-ui/run-20260520-153030/*.png` | The Details hero now requests focus on the primary Play action, moves between Play and Back to poster wall with `Left`/`Right`, and activates the focused action with `Enter`; the generated local-source GUI smoke opens Details from the poster wall, sends `Down`, captures `local-source-details-episodes.png` to prove focus can enter the TV-style episode shelf, sends another `Down`, captures `local-source-details-episode-selected.png`, then presses `Enter` and verifies Player receives Frieren episode 2. | Deeper Details-to-metadata panel focus paths can still be expanded later. |
...
`build/desktop-local-source-ui/run-20260520-153030/local-source-poster-keyboard.png`,
proving the Library poster wall can move selection with keyboard input before
opening Details, then send `Down` from the Details hero into
