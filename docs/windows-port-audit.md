...
with repeatable mpv/RIFE runtime gates, prior local mpv_PlayKit runtime
evidence, and a passing JSON-driven desktop behavior smoke report at
`build/desktop-behavior/run-20260527-070539/report.json`, plus a packaged
Windows WebUI HTTP smoke report at
`desktop-app/build/web-control-smoke/desktop-web-control-smoke.json`, but the
current checkout tracks only the placeholder `runtime/mpv` directory. Live
CloudDrive2 end-to-end QA, target-hardware RIFE validation with a prepared
runtime payload, signed release installer evidence, and deeper desktop-vs-TV UI
parity for less-traveled keyboard/DPAD focus paths remain open.
...
Release packaging now has a packaged runtime gate that builds
`desktop-app-<windowsPackageVersion>.zip`, launches the extracted packaged
runtime with `mpv.exe --version`, and verifies the
...
local Cloud/RSS scheduler smoke on `codex/**` push branches, writes and asserts
the scheduler JSON report, and uploads that token-free evidence. A separate
`windows-latest` job builds and uploads a versioned lightweight Windows desktop
ZIP, runs `:desktop-app:test`, `:desktop-app:desktopWebControlSmoke`,
`:desktop-app:desktopBehaviorTest`, and `:desktop-app:distZip`, asserts both
WebUI and desktop behavior reports, and
uploads the WebUI report plus desktop behavior `report.json`/screenshots;
nightly main/master releases now run the same lightweight WebUI/behavior smoke
before attaching the ZIP alongside the Android APK and upload workflow evidence
without treating behavior evidence as a release artifact.
...
`core:model` now also owns duration-aware playback position clamping through
`PlaybackTimingConventions`, replacing the Android TV-only seek clamp previously
embedded in `PlayerViewModel`. Android TV player controls, Android WebUI
start/progress/command/status DTOs, Windows WebUI start/progress/command/status
DTOs, and desktop mpv session syncing now reuse that same rule, while
`:repository-api` clamps explicit playback-source start overrides and
`web-control-core` resolves WebUI play-request starts, progress/detail/continue
watching DTO positions, absolute/relative/skip command targets, and reported
WebUI status positions with the known duration. The focused command gate passed
in Gradle MCP build `b-269` with 23 tests, the touched-module gate
`:core:model:test :web-control-core:test :desktop-app:test
-PbundleMpvRuntime=false` passed in `b-270` with 480 tests, the
Android/WebUI/desktop compile/separation gate passed in `b-271`, the focused
status gate passed in `b-273` with 7 tests, the broader WebControl/Android
compile/desktop bridge gate passed in `b-274` with 101 tests, the focused
...
`:repository-api` now also owns `savePlaybackProgressOnCompletion`, so Android
TV Media3 ended playback and Windows mpv ended playback use the same completion
progress-save rule: prefer a positive duration, fall back to observed player
position, then fall back to the session estimate, and increment play count
through the shared snapshot path. The former desktop completion-save helper is
now a thin adapter over the shared repository API. `PlaybackProgressSyncTest`
covers duration-first, observed-position fallback, session fallback, and
nonpositive-duration fallback behavior, while `DesktopPlaybackStopHandlerTest`
continues to cover the Windows adapter. The focused
repository/desktop/Android gate passed in Gradle MCP build `b-282` with 16
tests and Android debug assemble.
`core:model` now also owns shared playback progress-fraction math through
`PlaybackTimingConventions.playbackProgressFraction`. Android TV player chrome,
Android continue-watching poster progress, and the Windows mpv playback stage
use that same duration-aware clamp instead of inline `position / duration`
math. The Windows stage now keeps the configured start label while idle and,
while mpv is active, updates the bottom timeline from the already-polled mpv
position/duration so it shows live current/duration labels plus a clamped
progress bar rather than a decorative zero-width bar. `PlaybackTimingConventionsTest`
...
focused shared/desktop/TV compile gate passed in Gradle MCP build `b-285` with
44 tests, and the wider `:core:model:test :desktop-app:test
:ui-tv:compileDebugKotlin :app:assembleDebug checkDesktopPresenterSeparation
checkDesktopComposeOnly -PbundleMpvRuntime=false` preservation gate passed in
build `b-286` with 401 tests.
...
playback settings, and `core:model` owns duration-aware playback position
clamping used by Android TV controls plus Android/Windows WebUI playback
commands and status DTOs. The
scan-preference contract used by Android TV
...
| Plan and integrate a bundled RIFE-capable mpv runtime | `runtime/mpv`, `tools/prepare-mpv-runtime.ps1`, `desktop-app:verifyMpvRuntimePayload`, `smokeMpvRuntime`, `smokePackagedMpvRuntime`, `smokeNativeAppImageRuntime`, `packageWindowsAppImage`, `tools/smoke-mpv-rife.ps1`, `tools/assert-mpv-rife-report.ps1`, `docs/mpv-runtime-packaging.md` | Runtime verifier and Gradle smoke pass for NVIDIA/DIRECTML scripts; `MpvRuntimeVerifier` now cross-checks `runtime-manifest.json` declared files/directories and required RIFE backends, rejects malformed manifests, unknown backend names, and absolute/`..` manifest entries as incomplete evidence, and the desktop runtime panel localizes that guidance. Gradle runtime gates now use the same manifest-evidence checks for source runtime payloads, packaged zip entries, and native app-image runtime content. `:desktop-app:smokePackagedMpvRuntime -PmpvRuntimeSource=runtime\mpv -PrequireMpvRuntime=true -PrequiredRifeBackends=NVIDIA,DIRECTML` builds `desktop-app-<windowsPackageVersion>.zip`, verifies the bundled zip contains `runtime/mpv/mpv.exe`, `runtime-manifest.json`, the NVIDIA/DIRECTML scripts, and manifest-declared entries, then extracts the zip and launches the packaged `runtime/mpv/mpv.exe --version`. `:desktop-app:smokeNativeAppImageRuntime -PmpvRuntimeSource=runtime\mpv -PrequireMpvRuntime=true -PrequiredRifeBackends=NVIDIA,DIRECTML` builds the Windows `jpackage` app image, verifies `MiruPlay.exe`, validates `app/MiruPlay.cfg` main class/classpath entries, checks the app-image `runtime/mpv` payload and manifest-declared entries, and launches the app-image entry in headless smoke mode to verify the resolved runtime paths before installer/signing work. Prior DirectML VapourSynth/RIFE smoke proved the payload path; `-Backend ALL -AllowFailures` reports the backend matrix. `tools/smoke-mpv-rife.ps1 -ReportPath ...` writes a JSON evidence bundle with mpv version, host OS/CPU/GPU diagnostics, backend statuses, exit codes, log paths, and runtime manifest evidence for target-host QA while preserving arguments with spaces when launching mpv. `tools/assert-mpv-rife-report.ps1 -RequireRuntimeManifest` validates those JSON reports for host diagnostics, clip shape, backend entries, required backend PASS status, and a present/problem-free manifest that declares the required backend(s); `tools/verify-windows-port.ps1 -Rife` now runs that stricter validation against the selected `-MpvRuntimeSource` immediately after generating the report. | Local RIFE playback is non-blocking on this host; backend performance/compatibility must be validated on target hardware. |
...
| Provide auditable verification gates | Gradle MCP build records, `.github/workflows/ci.yml`, scripts under `tools/`, this audit document | CI now runs the Android debug build plus desktop/shared JVM checks on mainline branches and `codex/**` push branches: `checkDesktopComposeOnly`, `checkDesktopPresenterSeparation`, `checkUiPaletteDrift`, `:core:model:test`, `:media-source-api:test`, `:metadata-core:test`, `:repository-api:test`, `:cloud-drive-api:test`, `:sync-engine-shared:test`, `:media-source-desktop:test`, `:scanner-desktop:test`, `:repository-desktop:test`, `:scraper-desktop:test`, `:player-mpv:test`, `:cloud-drive-desktop:test`, `:sync-engine-desktop:test`, `:sync-engine-desktop:smokeCloudDriveRssScheduler`, `:desktop-app:test`, and lightweight `:desktop-app:installDist -PbundleMpvRuntime=false`. A Windows runner also builds and uploads a versioned lightweight ZIP through `:desktop-app:test :desktop-app:desktopWebControlSmoke :desktop-app:desktopBehaviorTest :desktop-app:distZip -PdesktopBehaviorTags=smoke -PwindowsPackageVersion=... -PbundleMpvRuntime=false`, and uploads WebUI smoke plus behavior report/screenshots; nightly main/master releases attach that same-version ZIP alongside the Android APK without publishing behavior evidence as a release artifact. `tools/verify-windows-port.ps1` now auto-selects JDK 21 when launched from a newer JDK shell and the safe default gate includes packaged WebUI smoke, the matching WebUI JSON assertion, and the token-free Cloud/RSS scheduler elapsed-time smoke; `-Behavior` runs and asserts the same JSON-driven desktop behavior smoke used by Windows CI. `DesktopRepositoriesTest` now recursively removes its own temporary store directory, avoiding Windows `DirectoryNotEmptyException` cleanup flakes in the repository persistence gate. Latest local commands are listed below with passing evidence for Android build, desktop tests, packaged WebUI smoke, desktop behavior smoke, mpv tests, CloudDrive loopback tests, scheduler smoke, runtime smoke, packaged runtime zip smoke, DirectML RIFE smoke, and screenshot QA. | Hardware/cloud/live-service checks are intentionally tracked as not achieved. |
...
| Real mpv executable can launch | mpv_PlayKit `20260510` assets were downloaded into `.gradle/mpv-playkit-20260510`; a prior local `runtime/mpv` payload was prepared from `mpv-lazy-20260510.exe` plus the `mpv-lazy-20260510-vsNV.7z.001` overlay, then the default `:desktop-app:smokeMpvRuntime -PrequireMpvRuntime=true` gate passed with `mpv v0.41.0-615-g7b057f66f` and required RIFE `NVIDIA, DIRECTML`. The current Git checkout keeps only the placeholder directory, so this evidence requires supplying or preparing that runtime payload again. | Covered by gates and prior local evidence when a payload is supplied |
| Bundled RIFE runtime is supported | Runtime layout expects `portable_config/vs/MEMC_RIFE_NV.vpy` and `MEMC_RIFE_DML.vpy` for the default release gate; app can also select the optional Standard script when present. The verifier blocks launch when a selected script is missing, and the runtime manifest is now checked against actual packaged relative files/directories instead of being trusted as display-only metadata. A prepared runtime manifest records the standard `.exe` base plus `vsNV` overlay and the default NVIDIA/DirectML requirements; the current checkout only keeps the `runtime/mpv/README.MD` placeholder and no `runtime-manifest.json`. | Covered structurally when a runtime payload is supplied |
...
| Desktop distribution runtime copy is controllable | `desktop-app/build.gradle.kts` bundles exactly one runtime source: explicit `-PmpvRuntimeSource` when present, otherwise repository `runtime/mpv` only if it contains `mpv.exe`. `bundleMpvRuntime` defaults to `true` for self-contained artifacts, while `-PbundleMpvRuntime=false` skips the large runtime copy for UI-only development installs; verified with `:desktop-app:installDist -PbundleMpvRuntime=false` and, when a prepared runtime payload is supplied, `:desktop-app:smokePackagedMpvRuntime -PmpvRuntimeSource=runtime\mpv -PrequireMpvRuntime=true -PrequiredRifeBackends=NVIDIA,DIRECTML`, which builds `desktop-app-<windowsPackageVersion>.zip`, extracts it, smokes the packaged `runtime/mpv/mpv.exe --version`, and checks the packaged runtime entries plus manifest-declared files/directories. | Covered when a runtime payload is supplied |
...
| Desktop WebUI access state | `repository-desktop` provides `FileBackedWebControlAccessManager`, implementing the shared `WebControlAccessManager` interface with persisted enable state, URL-safe token generation/rotation, and enabled-change listeners in the desktop JSON store. `core:common` owns `findWebControlLocalIps()` and `buildWebControlAccessUrls()`, and Android TV `SettingsViewModel` reuses the same URL builder instead of keeping its own `NetworkInterface`/token-encoding logic. `:web-control-core` now owns shared NanoHTTPD routing, auth/cookie handling, API envelopes/DTOs, static serving, request-encoding helpers, server-info/IP DTO mapping, source request shaping/password redaction/add-test-update-remove persistence, source-test response wording, source-scan response mapping, shared Result error handling, local-directory DTO mapping, duration-clamped progress/detail DTO mapping, CloudDrive automation DTO mapping, CloudDrive/RSS request validation, config/login/token/run service glue, RSS subscription persistence flow, and response shaping, CloudDrive directory browse flow and DTO mapping over the shared directory-browser state, library DTO shaping, library search filtering, duration-clamped WebUI play-request start-position/source payload mapping, duration-clamped WebUI playback status DTO mapping, and duration-aware command seek-target resolution, so Android `WebControlServer` is a thin asset-backed wrapper and Windows `DesktopWebControlServer` reuses the same HTTP surface with classpath WebUI assets. Compose Desktop Settings starts/stops that server from the shared enable state; the desktop service currently serves token-protected info, source list, local directory browsing, library search/detail, CloudDrive config summary, playback status, playback start/control, static WebUI bootstrap, basic source/RSS/config writes, real local/WebDAV/SMB source connection tests with shared user-facing response text, source scans through the shared `scanAndIndexDesktopSource` helper used by manual desktop scans and Cloud/RSS linked rescans, CloudDrive login/token verification through the shared directory-browser prepare/load flow plus `DesktopCloudDriveRssAutomationEngine`, scoped CloudDrive directory browsing through the shared directory-browser helper, and WebUI-triggered CloudDrive/RSS manual runs. WebUI library/detail grouping now delegates to shared media-index poster groups and honors the persisted same-anime merge preference, matching the Windows poster wall behavior for externally matched seasons. Playback requests are injected by Compose Desktop through `DesktopWebControlPlaybackBridge.kt` into `DesktopPlaybackLauncher` and `desktopWebControlPlaybackCommand`, preserving episode ids for progress while retaining real media paths and remote source lifetimes for mpv loopback streaming; playback starts, progress payloads, commands, and reported status positions now clamp over-duration targets through the same `core:model` rule used by Android TV. `DesktopWebControlPlaybackBridgeTest` covers WebUI command status text, source selection/reuse/ownership, duration-clamped seek commands, and next-episode temporary-source inheritance; `WebControlPlaybackStatusTest`, `WebControlPlaybackTest`, `WebControlProgressTest`, and `EpisodePlaybackSourceResolverTest` cover duration-clamped status DTOs, explicit starts, progress DTOs, and shared playback-source construction. `DesktopWebControlServerTest` covers auth, static cookie, disabled state, source redaction, repository-backed library/detail data, shared poster merge preference for WebUI library/detail, single-source WebUI scan, scan-all WebUI scan, post-scan library visibility, CloudDrive login/token persistence, directory scoping/filtering, run-once RSS submission through the shared desktop engine, injected WebUI playback play/command handling, and the packaged-smoke launcher contract. `:desktop-app:desktopWebControlSmoke -PbundleMpvRuntime=false` passed in Gradle MCP build `b-24` and writes `desktop-app/build/web-control-smoke/desktop-web-control-smoke.json`, proving the installed Windows app script can serve the packaged WebUI shell, require API auth, set cookie auth from static bootstrap, redact source secrets, expose library/detail progress, serve CloudDrive summary, and dispatch playback play/command APIs. | Covered for packaged local WebUI; LAN/browser visual QA remains manual |
| Playback progress continues on desktop | Compose Desktop saves progress when mpv launches, polls mpv `time-pos` every 10 seconds for persisted progress and every second for the visible playback stage while playback is active, saves a session-estimated position immediately on Stop so process shutdown is not blocked by IPC, shows continue-watching records, can clear a selected recent item, provides TV-style Play/Pause/-10s/+30s/Stop controls in the playback stage with left/right focus movement, Enter activation, and Up/Down movement to/from `返回详情`, adds unit-covered movement from playback-setting fields through toggles/backend into the runtime card and back, adds unit-covered vertical movement through runtime path fields and `检查运行时`, drives native desktop fullscreen from the Player `全屏` toggle while preserving route-based restore behavior, and stores original remote path rather than loopback URL. Android TV and Windows now both resolve Play-next targets through `repository-api` `buildNextPlaybackSource`, share `savePlaybackProgressOnCompletion` for ended-playback completion progress/play-count persistence, and Android TV/WebUI plus Windows WebUI start/progress/command/status DTOs share `core:model` playback-position clamping so launch positions, progress payloads, command targets, and reported positions cannot drift past known media duration. Android TV player timelines, Android continue-watching poster progress, and the Windows mpv stage also share `PlaybackTimingConventions.playbackProgressFraction`, so the desktop stage now shows live current/duration labels and a clamped active progress bar instead of a decorative zero-width timeline. `NextPlaybackSourceResolverTest` covers shared ordering, resume, completed-progress reset, null handling, and platform-specific playable URI mapping; `PlaybackProgressSyncTest` covers shared start/stop/completion progress snapshots and completion duration/observed/session fallback; `ProgressExtensionsTest`, `EpisodePlaybackSourceResolverTest`, `WebControlPlaybackTest`, `WebControlProgressTest`, `PlaybackTimingConventionsTest`, `WebControlPlaybackCommandTest`, `WebControlPlaybackStatusTest`, `DesktopPlaybackPanelTest`, and `DesktopWebControlPlaybackBridgeTest` cover the shared clamp/fraction from core through TV/desktop timeline surfaces, WebUI start/progress/status/command, and desktop mpv command execution. `tools/smoke-desktop-mpv-launch-ui.ps1` now proves the GUI can launch a generated local sample through mpv, exercise Pause/-10s/+30s/Stop, confirm the mpv process exits, and record `mpv-settings-focus.png`, `mpv-runtime-focus.png`, `mpv-launch-ready.png`, `mpv-launched.png`, `mpv-keyboard-controls-used.png`, and `mpv-stopped.png` evidence with a 30s saved position. | Covered |
...
`.\tools\verify-windows-port.ps1 -SkipAndroidBuild -Behavior`; it covered the
safe Gradle gate, packaged WebUI smoke report assertion, desktop behavior smoke,
and CloudDrive RSS scheduler smoke. The behavior report is
`build/desktop-behavior/run-20260527-070539/report.json`, and the scheduler
report is `build/cloud-rss-smoke/scheduler-report.json`. Packaged WebUI smoke
passed in Gradle MCP build `b-24`; its local report is
