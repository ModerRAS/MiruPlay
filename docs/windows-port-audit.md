...
Windows app-image directory, verifies the native launcher, and checks the
bundled `runtime/mpv` payload plus NVIDIA/DIRECTML scripts before installer
signing work starts. `tools/verify-windows-port.ps1` now wraps the local port gate with
