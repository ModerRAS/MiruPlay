...
support horizontal `Left`/`Right` focus movement and `Enter` activation. Compose Desktop now treats `Esc`, TV remote `Back`, and navigation back/out keys as Back equivalents for Player -> Details -> Library and Settings -> Library while leaving `Backspace` to text fields. The remote browser list now supports `Up`/`Down` row movement, first-row
