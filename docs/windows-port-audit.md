...
commands, Windows WebUI commands, and desktop mpv session syncing now reuse that
same rule, while `web-control-core` resolves absolute/relative/skip command
targets with each platform target's current duration. The focused seek-clamp
gate passed in Gradle MCP build `b-269` with 23 tests, the touched-module gate
`:core:model:test :web-control-core:test :desktop-app:test
-PbundleMpvRuntime=false` passed in `b-270` with 480 tests, and the
Android/WebUI/desktop compile/separation gate passed in `b-271`.
...
playback settings, and `core:model` owns duration-aware playback seek clamping
used by Android TV controls plus Android/Windows WebUI playback commands. The
