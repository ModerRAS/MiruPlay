...
desktop-vs-TV UI parity beyond the Library, Details, and Player first screens
remain open.
...
| Playback progress continues on desktop | Compose Desktop saves progress when mpv launches, polls mpv `time-pos` every 10 seconds while playback is active, queries mpv again on stop when IPC responds, falls back to `DesktopPlaybackSession` estimation, shows continue-watching records, can clear a selected recent item, provides TV-style Play/Pause/-10s/+30s/Stop controls in the playback stage, and stores original remote path rather than loopback URL. | Covered |
...
1. Continue narrowing full Android TV screen parity gaps beyond the Library,
   Details, and Player first screens, especially Settings, source management,
   and navigation shape.
