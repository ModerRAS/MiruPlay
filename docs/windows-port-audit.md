...
|---|---|---|---|
| Keep the original Android TV app working | `app/src/main/kotlin/com/miruplay/tv/MainActivity.kt`, `ui-tv/`, Android Gradle modules | `:app:assembleDebug` passed in the latest preservation check. Emulator `10.137.32.118:5555` was launched with a local fixture via `test_local_path`, and `build/android-tv-qa/library-fixture-20260519134617.png` captures the TV Library as a large "最高热度 / 最近添加 / 海报墙" media wall. | Deeper detail/player TV-device flow still needs a focused run. |
