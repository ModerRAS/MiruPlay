...
deeper desktop-vs-TV UI parity for source-management/settings subflows remains
open.
...
|---|---|---|
| Android TV remains buildable | `.\gradlew.bat :app:assembleDebug` passed in the latest preservation check after the desktop port work. `.\tools\smoke-android-tv-ui.ps1` also installed and launched the debug APK on emulator `10.137.32.118:5555`. | Covered for debug build and current device smoke |
...
1. Continue narrowing TV parity gaps in source-management subflows,
   especially long-path and focus behavior.
