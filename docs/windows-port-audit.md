...
with a real mpv_PlayKit RIFE-capable runtime prepared locally and a passing
JSON-driven desktop behavior smoke report at
`build/desktop-behavior/run-20260526-142805/report.json`, plus a packaged
Windows WebUI HTTP smoke report at
`desktop-app/build/web-control-smoke/desktop-web-control-smoke.json`, but live
CloudDrive2 end-to-end QA, target-hardware RIFE validation, signed release
installer evidence, and deeper desktop-vs-TV UI parity for less-traveled
keyboard/DPAD focus paths remain open.
...
| Desktop distribution runtime copy is controllable | `desktop-app/build.gradle.kts` bundles exactly one runtime source: explicit `-PmpvRuntimeSource` when present, otherwise repository `runtime/mpv`. `bundleMpvRuntime` defaults to `true` for self-contained artifacts, while `-PbundleMpvRuntime=false` skips the large runtime copy for UI-only development installs; verified with `:desktop-app:installDist -PbundleMpvRuntime=false` and `:desktop-app:smokePackagedMpvRuntime -PmpvRuntimeSource=runtime\mpv -PrequireMpvRuntime=true -PrequiredRifeBackends=NVIDIA,DIRECTML`, which builds `desktop-app-<windowsPackageVersion>.zip`, extracts it, smokes the packaged `runtime/mpv/mpv.exe --version`, and checks the packaged runtime entries plus manifest-declared files/directories. | Covered |
