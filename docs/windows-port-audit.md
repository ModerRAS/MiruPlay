...
commands/status DTOs, Windows WebUI commands/status DTOs, and desktop mpv
session syncing now reuse that same rule, while `web-control-core` resolves
absolute/relative/skip command targets and reported WebUI status positions with
the known duration. The focused command gate passed in Gradle MCP build `b-269`
with 23 tests, the touched-module gate `:core:model:test :web-control-core:test
:desktop-app:test -PbundleMpvRuntime=false` passed in `b-270` with 480 tests,
the Android/WebUI/desktop compile/separation gate passed in `b-271`, the
focused status gate passed in `b-273` with 7 tests, and the broader
WebControl/Android compile/desktop bridge gate passed in `b-274` with 101 tests.
