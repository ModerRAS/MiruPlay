...
Latest playback seek-clamp sharing update: `core:model`
...
Android WebUI command targets, Windows WebUI command targets, and desktop mpv
session syncing now reuse that same rule; `web-control-core` asks each platform
target for duration before resolving absolute, relative, skip-forward, and
skip-backward command targets. The focused gate `:core:model:test --tests
