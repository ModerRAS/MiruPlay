...
Android WebUI command/status targets, Windows WebUI command/status targets, and
desktop mpv session syncing now reuse that same rule; `web-control-core` asks
each platform target for duration before resolving absolute, relative,
skip-forward, and skip-backward command targets, and WebUI playback status DTOs
also clamp reported position by known duration. The focused command gate
...
passed in `b-273` with 7 tests, and the broader WebControl status/Android
compile/desktop bridge gate `:web-control-core:test :web-control:compileDebugKotlin
:desktop-app:test --tests com.miruplay.tv.desktop.DesktopWebControlPlaybackBridgeTest
-PbundleMpvRuntime=false` passed in `b-274` with 101 tests.
