...
| Release packaging | Partial | Lightweight install works; full bundled runtime `distZip` now has a packaged mpv/RIFE smoke gate, and the Windows app-image path has a JDK `jpackage --type app-image` gate that verifies the launcher plus bundled mpv/RIFE runtime. `tools/verify-windows-port.ps1` now provides a repeatable local gate with safe defaults, automatically selects JDK 21 on Windows when available, and keeps GUI, real-library, Android TV, SMB, mpv-runtime, native app-image, and RIFE checks opt-in. GitHub Actions now runs the Android/desktop/shared JVM CI gate on `codex/**` push branches as well as mainline branches, while nightly/release publishing is explicitly limited to main/master scheduled or manual runs. Current `codex/windows-mpv-rife` CI run `26212495116` passed build/lint on commit `88fcd3c`, and skipped `nightly`/`build-release` as intended. MSI/EXE installer signing and target-host runtime validation remain open. |
...
- [x] Add a Windows native app-image gate that builds with JDK `jpackage` and verifies the launcher plus bundled mpv/RIFE runtime.
...
- [x] Keep CI green for Android and desktop/shared JVM gates on the active `codex/windows-mpv-rife` branch (`CI Build` run `26212495116` passed build/lint on commit `88fcd3c`).
