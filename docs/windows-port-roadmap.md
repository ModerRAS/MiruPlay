...
| Area | Status | Notes |
|---|---|---|
| Android TV build | Covered for debug build and DPAD Library/detail/player/settings smoke | Latest local `.\gradlew.bat :app:assembleDebug -PbundleMpvRuntime=false` passed on 2026-05-27. `tools/smoke-android-tv-ui.ps1 -KeepAppData` installs the debug APK on device `<android-tv-device-id>`, generates a playable fixture library, scans it through the TV UI, verifies Library -> Details -> Player -> Library Back behavior, enters Settings, opens the media-source panel, proves source-card focus/edit behavior, removes only the current smoke source without clearing unrelated app data, then visits Playback, CloudDrive, Scan, and Metadata settings. The current report is `build/android-tv-qa/run-20260527-122708/android-tv-smoke-report.json` with 16 screenshots and 17 XML dumps. |
...
| Release packaging | Partial | Lightweight install works; full bundled runtime `distZip` now has a packaged mpv/RIFE smoke gate, and the Windows app-image path has a JDK `jpackage --type app-image` gate that verifies the launcher, generated launcher config/classpath, bundled mpv/RIFE runtime, and a real `MiruPlay.exe` headless desktop-entry smoke report from the generated app image through `tools/assert-desktop-entry-smoke-report.ps1`. Runtime packaging now treats a runtime source as present only when `mpv.exe` exists, so the tracked `runtime/mpv/README.MD` placeholder cannot accidentally trigger an incomplete bundled runtime build. The latest master-safe local Gradle gate passed on `master` in Gradle MCP build `b-1` with 264 tests passed, 0 failed, `:desktop-app:desktopWebControlSmoke` passed, and 493 actionable tasks completed in 3m 2s; it covered `:app:assembleDebug`, the desktop Compose/presenter/palette checks, shared JVM module tests, desktop module tests, `:desktop-app:installDist`, and packaged WebUI smoke with `-PbundleMpvRuntime=false`. The latest local full desktop behavior verifier passed on 2026-05-27 with `.\tools\verify-windows-port.ps1 -SkipGradle -SkipCloudRssScheduler -Behavior -BehaviorTags full`, producing `build/desktop-behavior/run-20260527-111726/report.json`; the latest local unsigned completion audit now accepts that full desktop behavior report alongside packaged WebUI, Bangumi live, Android TV device smoke, prepared mpv/RIFE runtime, Cloud/RSS scheduler evidence, and bundled-runtime SFX EXE installer evidence, but still fails on five target/live-service evidence items: target NVIDIA RIFE, CloudDrive2 live, and RSS dry-run/live-submit/organize. Strict release completion also still needs signed installer evidence. `tools/check-windows-port-external-prereqs.ps1` and `tools/verify-windows-port.ps1 -ExternalPrereqAudit` now write `build/windows-port-audit/external-prereqs.json` so those external blockers are visible without making live calls. The Windows start script now uses `%APP_HOME%\lib\*` instead of expanding every jar into the Java command line, avoiding command-length startup failures in installDist ZIPs. CI now also has a `windows-latest` job that builds `:desktop-app:test :desktop-app:distZip -PwindowsPackageVersion=... -PbundleMpvRuntime=false`, runs `:desktop-app:desktopWebControlSmoke -PbundleMpvRuntime=false` and `:desktop-app:desktopBehaviorTest -PdesktopBehaviorTags=smoke -PbundleMpvRuntime=false`, and uploads the versioned Windows desktop ZIP plus WebUI/desktop behavior evidence; nightly main/master releases attach the same-version lightweight ZIP alongside the Android APK, but nightly does not publish the behavior evidence as a release artifact. The `packageWindowsAppImage`, `distZip`, `distTar`, and installer paths now all use the same `windowsPackageVersion` input instead of mixing a hard-coded app-image version with installer metadata. `:desktop-app:smokeWindowsInstaller -PrequireWindowsInstallerToolchain=true` remains the opt-in MSI/EXE gate: it preflights the selected jpackage/WiX or SFX backend, reuses the verified app image, emits installer SHA256/size/version/backend/signing-mode JSON evidence, and can sign plus verify with explicit `signtool`/PFX inputs. `tools/assert-windows-installer-report.ps1` validates the installer evidence JSON against the generated MSI/EXE path, size, SHA256, type, backend, app version, and expected signing mode, and `tools/verify-windows-port.ps1 -WindowsInstaller` runs that assertion after packaging. MSIX/AppX/App Installer publishing is explicitly outside the current completion scope; the current Windows installer route is MSI/EXE, with large bundled-runtime releases using EXE/SFX unless the product release target changes. `tools/verify-windows-port.ps1` now provides a repeatable local gate with safe defaults, automatically selects JDK 21 on Windows when available, and keeps GUI, JSON-driven behavior, real-library, Android TV, SMB, mpv-runtime, native app-image, Windows installer, RIFE, and external prerequisite checks opt-in while running the local Cloud/RSS scheduler smoke by default. GitHub Actions now runs the Android/desktop/shared JVM CI gate on `codex/**` push branches as well as mainline branches, while nightly/release publishing is explicitly limited to main/master scheduled or manual runs. Signed release installer artifact validation and target-host runtime validation remain open. |
...
- [x] Assert the selected Windows installer backend in installer, external-prerequisite, and completion-audit evidence so SFX and jpackage reports cannot be mixed up.
- [ ] Run the signed bundled-runtime Windows installer gate with the release certificate and save installer evidence.
- [x] Record the current Windows release route as MSI/EXE, with large bundled-runtime releases using EXE/SFX; MSIX/AppX/App Installer is out of current completion scope unless the product release target changes.
...
# Strict completion audit. This does not run live hardware/service actions, but writes
