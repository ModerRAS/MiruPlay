...
| Area | Status | Notes |
|---|---|---|
| Android TV build | Covered for debug build and DPAD Library/detail/player smoke | Latest local `:app:assembleDebug` passed. `tools/smoke-android-tv-ui.ps1` installs the debug APK on emulator `10.137.32.118:5555`, generates a playable fixture library, scans it through the TV UI, confirms Library content requests poster focus, drives DPAD Right/Left then Center from the poster surface into Details, drives DPAD Center from Details into Player, and records `build/android-tv-qa/run-20260520-174116/android-tv-library.png`, `android-tv-library-dpad-poster.png`, `android-tv-details.png`, `android-tv-player.png`, XML dumps, and a JSON report. |
...
- [x] Add Android TV smoke coverage for Library poster focus and DPAD poster -> Details -> Player.
