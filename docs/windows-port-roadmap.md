...
| Library/index/details | Implemented foundation | Local scan/index, WebDAV scan/index, SMB scan/index, clear-index/remove-source, long-path saved-source display, TV-style poster-wall selection, details hero, and player handoff GUI smoke now passes for generated fixtures; local smoke also passes against `D:\Software\dufs` with the scanned Library opening directly on the poster wall. |
...
1. Continue narrowing deeper desktop-vs-Android-TV UI gaps beyond the first screens, especially remaining remote/source-management edit forms and keyboard/DPAD focus paths.
