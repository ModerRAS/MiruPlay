...
| Shared UI palette and labels | Covered structurally | `:ui-design` owns shared palette; drift check exists. `core:model` now also owns media-source type labels, default names, generic fallback names, source hints, location field labels, and shared settings section titles/descriptions/orderings used by Android TV settings and Windows source/settings UI. |
