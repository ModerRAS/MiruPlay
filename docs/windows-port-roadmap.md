...
- [x] Add Android TV smoke coverage for Library poster focus and DPAD poster -> Details episode row -> Player -> Back to Library poster wall -> Settings -> media sources -> source card -> menu return -> source edit form -> source delete button -> empty source state.
