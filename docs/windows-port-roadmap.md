...
| WebUI desktop access | Partial | Windows Settings now includes the same first-class `WebUI` settings category as Android TV instead of hiding it from the desktop section order. `repository-desktop` implements the shared `WebControlAccessManager` contract with file-backed enable state, URL-safe random access-token generation/rotation, persistence across restarts, and enabled-change listeners. `core:common` owns shared LAN-IP discovery and WebUI access URL construction, so Android TV `SettingsViewModel` and the Windows settings surface use the same default port, token encoding, address filtering, and duplicate handling. The desktop Settings WebUI page now shows Windows as the platform, renders shared WebUI summary tiles/status copy, exposes enable/disable, token rotation, refresh-address actions, and previews the generated token/addresses honestly as access configuration while the JVM WebControl server is still pending. `:core:common:test`, `:core:model:test`, `:repository-desktop:test`, `:desktop-app:test`, and `:ui-tv:compileDebugKotlin` cover the shared helpers, desktop persistence, settings order, and Android reuse. |
