...
- [x] Add an opt-in Windows MSI/EXE installer gate that preflights WiX, builds from the verified app image, records installer SHA256/size/version/signing-mode evidence, and supports explicit signtool/PFX signing.
...
- [x] Keep CI green for Android and desktop/shared JVM gates on the active `codex/windows-mpv-rife` branch (recent recorded `CI Build` run `26217656025` passed build/lint on commit `160f656`).
