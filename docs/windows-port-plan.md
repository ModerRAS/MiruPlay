...
- 新增 `:web-control-core` 纯 JVM 模块，承接 Android WebControl 的共享 DTO、UTF-8 请求解析、NanoHTTPD 路由、访问令牌鉴权、cookie bootstrap 和静态 WebUI 服务逻辑；Android 端 `WebControlServer` 现在只是 Context assets 包装，Windows 端则复用同一 HTTP 层和同一套 WebUI assets。桌面版已能随 Settings WebUI 开关启动 JVM 监听器并服务信息、片库、详情、媒体源、CloudDrive 配置摘要、播放状态、扫描、CloudDrive 目录浏览/登录/执行，以及通过 `DesktopWebControlPlaybackBridge.kt` 和 `DesktopPlaybackLauncher` 接入的 WebUI 播放启动/控制；`:desktop-app:desktopWebControlSmoke -PbundleMpvRuntime=false` 会从安装版 Windows 启动脚本运行 WebUI HTTP smoke，覆盖静态 WebUI shell、token/cookie 鉴权、source redaction、library/detail、CloudDrive 摘要和 playback play/command API。剩余 WebControl 真实设备/LAN 浏览器 QA 仍按 roadmap 作为 parity 项推进。
- CI 的 debug build job 现在除了 Android `assembleDebug`，还会运行 `checkDesktopComposeOnly`、`checkDesktopPresenterSeparation`、`checkUiPaletteDrift`、`:core:model:test`、`:media-source-api:test`、`:metadata-core:test`、`:repository-api:test`、`:cloud-drive-api:test`、`:sync-engine-shared:test`、`:media-source-desktop:test`、`:scanner-desktop:test`、`:repository-desktop:test`、`:scraper-desktop:test`、`:player-mpv:test`、`:cloud-drive-desktop:test`、`:sync-engine-desktop:test`、`:desktop-app:test` 和轻量 `:desktop-app:installDist -PbundleMpvRuntime=false`，避免 Windows/JVM port 只在本地验证；Cloud/RSS scheduler smoke 会写出 JSON 并由 `tools/assert-cloud-rss-scheduler-report.ps1` 复核后上传。CI 还在 `windows-latest` 上构建并上传带 `windowsPackageVersion` 的轻量 `:desktop-app:distZip -PbundleMpvRuntime=false`，同时运行 `:desktop-app:desktopWebControlSmoke -PbundleMpvRuntime=false` 和 `:desktop-app:desktopBehaviorTest -PdesktopBehaviorTags=smoke -PbundleMpvRuntime=false`，用 `tools/assert-desktop-web-control-smoke-report.ps1` 与 `tools/assert-desktop-behavior-report.ps1` 复核 WebUI smoke、桌面行为 `report.json` 与截图证据后上传；nightly 主线发布也会先跑并复核同一组轻量 WebUI/behavior smoke，再把同版号 Windows desktop ZIP 追加到 GitHub Release，但不把行为测试证据伪装成 release artifact。完整 mpv/RIFE 自包含包仍走显式 runtime gate；轻量 MSI/EXE installer 可用 `-PbundleMpvRuntime=false` 做 QA，full `vsNV` bundled installer 走显式 `-PwindowsInstallerType=exe -PwindowsInstallerBackend=sfx` 后端，再配合 release 证书签名。
- 新增 `:player-mpv` JVM 模块，负责构造和启动 Windows mpv 命令：
  ...
  - 此前已在本机用 mpv_PlayKit `20260510` 标准 `mpv-lazy-20260510.exe` 作为 base，并叠加 `mpv-lazy-20260510-vsNV.7z.001` overlay 准备真实 `runtime/mpv` payload；`mpv.exe --version` smoke 输出版本为 `mpv v0.41.0-615-g7b057f66f`，`tools/smoke-mpv-rife.ps1 -Backend DIRECTML` 的两帧 Y4M 烟测已成功退出。当前 Git checkout 只保留占位目录，release gate 仍需要显式准备或提供这个 payload。
...
- 新增 `:sync-engine-desktop` JVM 模块：
  ...
  - `:sync-engine-desktop:smokeCloudDriveRssDryRun -PcloudDriveEndpoint=... -PcloudDriveToken=... -PcloudDriveRssUrl=... -PcloudDriveInbox=/Downloads -PcloudDriveLibrary=/Library -PcloudDriveRssReportPath=build/cloud-rss-smoke/dry-run-report.json` 可先在真实服务上验证 token、inbox/library listing、RSS fetch/parse、过滤规则和 would-submit 统计，并输出不含 token 的 JSON 证据报告；不会调用离线下载提交 API。`tools/assert-cloud-rss-report.ps1` 可校验 dry-run/live-submit/organize 报告的 inbox/library、候选统计、权限、提交/整理结果和脱敏状态。
  - `:sync-engine-desktop:smokeCloudDriveRssLiveSubmit -PcloudDriveEndpoint=... -PcloudDriveToken=... -PcloudDriveRssUrl=... -PcloudDriveInbox=/Downloads -PcloudDriveLibrary=/Library -PcloudDriveRssSubmitConfirmation=I_UNDERSTAND_THIS_SUBMITS_REAL_CLOUDDRIVE_DOWNLOADS -PcloudDriveRssSubmitLimit=1 -PcloudDriveRssReportPath=build/cloud-rss-smoke/live-submit-report.json` 会在真实 CloudDrive2 上提交有限数量的 RSS 候选离线下载并刷新 inbox listing，报告同样不会写入 token；这个任务有真实副作用，必须显式确认。`tools/assert-cloud-rss-scheduler-report.ps1` 可校验 scheduler elapsed-time 报告；`tools/verify-windows-port.ps1` 默认运行无外部副作用的 packaged WebUI smoke 和 scheduler smoke，`-SkipCloudRssScheduler` 仅用于临时跳过，`-CloudRssDryRun` 生成 `dry-run-report.json`，`-CloudRssLiveSubmit` 生成 `live-submit-report.json`，`-CloudRssOrganize -ConfirmCloudRssOrganize` 生成 `organize-report.json` 并要求真实移动证据，`-CloudRssEvidenceBundle -ConfirmCloudRssLiveSubmit -ConfirmCloudRssOrganize` 会按顺序生成 dry-run、live-submit、organize 三份报告。
