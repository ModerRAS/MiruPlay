...
class WebControlAutomationRequestsTest {
    ...
    fun `repository proxy helper reads shared proxy config`() = runBlocking {
        ...
        assertEquals("10.137.32.158", proxy.host)
    ...
    fun `repository proxy helper saves shared proxy without changing CloudDrive fields`() = runBlocking {
        ...
        assertEquals("10.137.32.158", config.rssProxyHost)
        assertEquals(65_535, config.rssProxyPort)
        assertEquals(NetworkProxyDto(enabled = true, host = "10.137.32.158", port = 65_535), proxy)
