...
import com.miruplay.tv.repository.savePlaybackProgressOnStop as sharedSavePlaybackProgressOnStop
...
internal suspend fun saveDesktopPlaybackProgressOnStop(
    session: DesktopPlaybackSession,
    queryPositionMs: (suspend () -> Result<Long?>)?,
    saveProgress: suspend (episodeId: String, positionMs: Long, lastWatched: Long) -> Result<Unit>,
    nowMillis: () -> Long = System::currentTimeMillis,
): Result<Long> =
    sharedSavePlaybackProgressOnStop(
        session = session,
        queryPositionMs = queryPositionMs,
        saveProgress = saveProgress,
        nowMillis = nowMillis,
