...
if ([string]::IsNullOrWhiteSpace($OutputRoot)) {
    $OutputRoot = Join-Path $repoRoot "build\desktop-smb-source-ui"
}
...
function Convert-UncPathToSmbUrl {
    param([string]$Path)

    $trimmed = $Path.TrimEnd("\")
    if ($trimmed -notmatch '^\\\\([^\\]+)\\([^\\]+)(?:\\(.*))?$') {
        throw "SmbBaseUrl was not provided and ShareTestPath is not a UNC path: $Path"
    }

    $server = $Matches[1]
    $share = $Matches[2]
    $rest = if ($Matches.Count -gt 3) { $Matches[3] } else { "" }
    $segments = @($share)
    if (-not [string]::IsNullOrWhiteSpace($rest)) {
        $segments += $rest.Split("\") | Where-Object { -not [string]::IsNullOrWhiteSpace($_) }
    }
    return "smb://$server/$($segments -join '/')"
}
...
if ([string]::IsNullOrWhiteSpace($ShareTestPath)) {
    throw "SMB smoke requires -ShareTestPath pointing at an approved writable test share directory."
}
